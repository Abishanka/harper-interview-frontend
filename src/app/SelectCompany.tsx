...
const SelectCompany = (
    { setSelectedCompany, pdfLoading, setPdfLoading, setPdfReady }: 
    { 
    ...
    useEffect(() => {
        const fetchCompanies = async () => {
            ...
            try {
                const response = await fetch("http://localhost:8000/api/fetch-company-list", {
                    method: "GET",
                });
    ...
    const handleFetchData = async () => {
        ...
        try {
            const response = await fetch(`http://localhost:8000/api/generate-form?company_id=${selectedCompany_}`, {
                method: "GET",
            });
            if (!response.ok) {
    ...
    const startPolling = async () => {
        ...
        const interval = setInterval(async () => {
            try {
                console.log("Checking PDF status for company:", selectedCompany_);
    ...
    return (
        <div className="flex flex-col w-full">
            <h1 className="text-2xl font-bold text-[#ff6d63]">Select a Company</h1>
            <div className="mb-4">
                {companyListLoading ? (
                    ...
                    >
                        ...
                        {companies.map((company: { id: string, company_name: string }, index: number) => (
                            <option key={`${company.id}-${index}`} value={company.id} className="text-black">
